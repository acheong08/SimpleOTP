...
func TestEntriesSave(t *testing.T) {
	database.SetPassword("01234567890123456789012345678901")
	...
	err := entries.Save()
	if err != nil {
		t.Errorf("Failed to save entries: %v", err)
	}

	// Verify that the saved file contains the expected data
	var savedEntries database.Entries
	err = savedEntries.Load()
	if err != nil {
		t.Errorf("Failed to load saved entries: %v", err)
	}

	// Compare the saved entries with the original entries
	if len(savedEntries.Entries) != len(entries.Entries) {
		t.Errorf("Unexpected number of saved entries. Expected %d, got %d", len(entries.Entries), len(savedEntries.Entries))
	}

	for name, encrypted := range entries.Entries {
		savedEncrypted, ok := savedEntries.Entries[name]
		if !ok {
			t.Errorf("Saved entries missing entry: %s", name)
			continue
		}
